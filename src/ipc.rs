...
use anyhow::Result;
use arrow::{
    array::{Int64Array, RecordBatch, TimestampMillisecondArray, UInt16Array, UInt64Array},
...
use polars::prelude::{CompatLevel, DataFrame, ParquetWriter, SchemaExt, Series, df, *};
...
pub(super) fn read_polars(path: impl AsRef<Path>) -> Result<(Metadata, DataFrame)> {
    let file = File::open(path)?;
